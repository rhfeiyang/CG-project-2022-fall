...
KdTreeNode::KdTreeNode(Grids_data &gridsData, uint32_t bm, wBBox bbox, int depth) : bitmap(bm), rightChild(nullptr),
                                                                                    leftChild(nullptr) {
    ...
    float pos_range[2] = {static_cast<float>(bbox.min()[partition_axis]),
                          static_cast<float>(bbox.max()[partition_axis])};
