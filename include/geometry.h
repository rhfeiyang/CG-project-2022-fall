...
//            tri.aabb = wBBox(vertices[tri.v_idx[0]], vertices[tri.v_idx[1]], vertices[tri.v_idx[2]]);
            tri.aabb.expand(vertices[tri.v_idx[0]]);
            tri.aabb.expand(vertices[tri.v_idx[1]]);
            tri.aabb.expand(vertices[tri.v_idx[2]]);
...
//                node->aabb = wBBox(tri.aabb, node->aabb);
        ...
        for (int axis = 0; axis < 3; ++axis) {
            std::sort(triangles.begin() + l, triangles.begin() + r + 1,
                      [&axis](const Triangle_index &A, const Triangle_index &B) -> bool {
...
//                aabb_l[i - l] = wBBox(aabb_l[i - l - 1], triangles[i].aabb);
                aabb_l[i - l].expand(aabb_l[i - l - 1]);
                aabb_l[i - l].expand(triangles[i].aabb);
            }
            for (int i = r - 1; i >= l; --i) {
//                aabb_r[i - l] = wBBox(aabb_r[i - l + 1], triangles[i].aabb);
                aabb_r[i - l].expand(aabb_r[i - l + 1]);
                aabb_r[i - l].expand(triangles[i].aabb);
            }
            float cost = INT_MAX;
            int split = l;
